...
module.exports = function(app) {
    ...
    app.post('/register/:token/:nickName', (req, res) => {
        const token = req.params.token
            ...
            .then(user => {
                if (user) return res.json({statusCode: 190, err: "해당 유저는 이미 DB에 등록되어 있습니다"})
                    ...
                    user.save( (err) => {
                      if (err) {
                        console.error(err)
                        return res.json({ resultCode: FAILURE, error: '유저를 등록할 수 없습니다. 이미 존재하는 닉네임입니다.' })
                      }
                      return res.json({ resultCode: SUCCESS, msg: '유저를 등록했습니다' })
                    })
                }
            })
            .catch(err => {
