...
module.exports = function(app) {
    app.get('/user/:token', (req, res) => {
        ...
        .then(user_id => {
            ...
            return res.status(StatusCode.OK).json({msg: "클라이언트의 토큰이 확인되었습니다", id: user_id})
        })
        .catch(reason => {
            console.log(" 에러 발생 ")
    ...
    app.get('/user/login/:token', (req, res) => {
        ...
        .then(user => {
            if(!user) return res.status(StatusCode.Forbidden).send("해당 유저가 존재하지 않습니다")
    ...
    app.post('/user/register/:token/:nickName', (req, res) => {
        ...
        .then(user => {
            if (user) return res.status(StatusCode.Conflict).send("해당 유저는 이미 가입되어 있습니다")
            else {
                let user = new User()
                user.nickName = req.params.nickName
                user.id = user_id
                user.save( (err) => {
                  if (err) throw new Error("유저를 DB에 저장할 수 없습니다\n"+ err)
    ...
    app.get('/user/check/:nickName', (req, res) => {
        User.findOne({ nickName: req.params.nickName }).exec()
        .then(isDuplicated => {
            if (isDuplicated) return res.status(StatusCode.Conflict).send("유저를 등록할 수 없습니다. 이미 존재하는 닉네임입니다")
            else return res.status(StatusCode.OK).send("사용하실 수 있는 닉네임입니다!")
        })
        .catch(reason => {
